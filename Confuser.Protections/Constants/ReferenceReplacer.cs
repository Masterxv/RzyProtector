	...
	internal class ReferenceReplacer {
        ...
        static void ReplaceNormal(MethodDef method, List<Tuple<Instruction, uint, IMethod>> instrs) {
			foreach (var instr in instrs) {
				...
				instr.Item1.Operand = "Rzy-Protector";
                method.Body.Instructions.Insert(i + 1, Instruction.Create(OpCodes.Ldc_I4, (int)instr.Item2 + new Random().Next(10000,100000)));
                method.Body.Instructions.Insert(i + 2, Instruction.Create(OpCodes.Ldc_I4, (int)instr.Item2));
                method.Body.Instructions.Insert(i + 3, Instruction.Create(OpCodes.Ldc_I4, new Random().Next(10000, 100000)));
